import { insertText, newlinesToSurroundSelectedText, expandSelectionToLine, clearExistingListStyle, makePrefix } from "../utils/Utils"
...
export default class ListStyle {
  ...
  static applyStyle(textarea: HTMLTextAreaElement, args: ListStyleArgs): void {
    ...
    const [undoResult, undoResultOppositeList, pristineText] = clearExistingListStyle(args, selectedText);
    ...
    if (noInitialSelection) {
      selectionStart = Math.max(selectionStart + makePrefix(0, unorderedList).length + newlinesToAppend.length, 0);
      selectionEnd = selectionStart;
    } else {
